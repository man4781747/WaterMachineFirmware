...
static inline bool _init_async_event_queue(){
    if(!_async_queue){
        ESP_LOGD("_init_async_event_queue", "準備建立駐列物件");
        _async_queue = xQueueCreate(128, sizeof(lwip_event_packet_t *));
        if(!_async_queue){
            ESP_LOGD("_init_async_event_queue", "建立駐列物件失敗!");
            return false;
        }
        else {
            ESP_LOGD("_init_async_event_queue", "建立駐列物件成功");
        }
    }
    else {
        ESP_LOGD("_init_async_event_queue", "列隊 _async_queue 已經初始化過，跳過");
    }
    return true;
}
...
static void _async_service_task(void *pvParameters){
    ...
    for (;;) {
        if(_get_async_event(&packet)){
            unsigned portBASE_TYPE queueLength = uxQueueMessagesWaiting(_async_queue);
            ESP_LOGD("", "列隊 _async_queue 目前還有 %d 個需求在駐列中等待", queueLength);
            // if ((int)queueLength > 0) {
            //     ESP_LOGD("", "列隊 _async_queue 目前還有 %d 個需求在駐列中等待", queueLength);
            // }
#if CONFIG_ASYNC_TCP_USE_WDT
    ...
    ESP_LOGD("_async_service_task", "Task 關閉");
    vTaskDelete(NULL);
...
static bool _start_async_task(){
    ...
    if(!_async_service_task_handle){
        ESP_LOGD("_start_async_task", "準備在CPU: %d 上建立 funvtion: _async_service_task 的TASK", CONFIG_ASYNC_TCP_RUNNING_CORE);
...
static int8_t _tcp_clear_events(void * arg) {
    ESP_LOGD("LwIP Callbacks", "_tcp_clear_events");
...
static int8_t _tcp_connected(void * arg, tcp_pcb * pcb, int8_t err) {
    //ets_printf("+C: 0x%08x\n", pcb);
    ESP_LOGD("LwIP Callbacks", "_tcp_connected");
    lwip_event_packet_t * e = (lwip_event_packet_t *)malloc(sizeof(lwip_event_packet_t));
...
static int8_t _tcp_accept(void * arg, AsyncClient * client) {
    ESP_LOGD("LwIP Callbacks", "_tcp_accept: 建立 lwip_event_packet_t 物件，並準備將此物件丟入TASK駐列");
    lwip_event_packet_t * e = (lwip_event_packet_t *)malloc(sizeof(lwip_event_packet_t));
    e->event = LWIP_TCP_ACCEPT;
    e->arg = arg;
    e->accept.client = client;
    if (!_prepend_async_event(&e)) {
        ESP_LOGD("LwIP Callbacks", "插入駐列失敗，清除空間");
        free((void*)(e));
    } else {
        ESP_LOGD("LwIP Callbacks", "插入駐列成功");
    }
    return ERR_OK;
}
...
void AsyncClient::onConnect(AcConnectHandler cb, void* arg){
    ESP_LOGD("AsyncClient", "onConnect");
    _connect_cb = cb;
    _connect_cb_arg = arg;
}
...
int8_t AsyncClient::_poll(tcp_pcb* pcb){
    ...
    uint32_t now = millis();
    // ESP_LOGD("AsyncClient", "_poll: _ack_timeout設定值: %d",_ack_timeout);
    // ACK Timeout
    if(_ack_timeout){
    ...
    // RX Timeout
    // ESP_LOGD("AsyncClient", "_poll: _rx_timeout設定值: %d",_rx_timeout);
...
void AsyncServer::onClient(AcConnectHandler cb, void* arg){
    //! 伺服器收到客戶端連線時的callback
    log_i("--- AsyncServer::onClient ---");
    _connect_cb = cb;
    _connect_cb_arg = arg;
}

void AsyncServer::begin(){
    ESP_LOGD("AsyncServer", "begin:準備初始化AsyncServer物件");
    if(_pcb) {
        return;
    }
    ...
    ESP_LOGD("AsyncServer", "準備設定tcp.h的tcp_accept CALLBACK");
...
int8_t AsyncServer::_accept(tcp_pcb* pcb, int8_t err){
    ESP_LOGD("AsyncServer", "_accept: 收到新連線數據，準備判斷是否需要處理");
    if(_connect_cb){
        ESP_LOGD("AsyncServer", "建立新 AsyncClient 物件");
...
int8_t AsyncServer::_accepted(AsyncClient* client){
    if(_connect_cb){
        ESP_LOGD("AsyncServer", "_accepted: class: AsyncServer 重新指向新的Client連線class物件");
        _connect_cb(_connect_cb_arg, client);
    } else {
        ESP_LOGD("AsyncServer", "我還不懂的步驟");
    }
    return ERR_OK;
}
...
int8_t AsyncServer::_s_accept(void * arg, tcp_pcb * pcb, int8_t err){
    ESP_LOGD("AsyncServer", "_s_accept: 由tcp.h的callback觸發: 轉換*arg的類型成為AsyncServer*，並繼續下一步");
    return reinterpret_cast<AsyncServer*>(arg)->_accept(pcb, err);
}

int8_t AsyncServer::_s_accepted(void *arg, AsyncClient* client){
    ESP_LOGD("AsyncServer", "_s_accepted: 轉換*arg的類型成為AsyncServer*，並繼續下一步");
