...
void AsyncWebServerRequest::send(AsyncWebServerResponse *response){
  _response = response;
  ESP_LOGD("AsyncWebServerRequest", "send!");
  if(_response == NULL){
    ESP_LOGD("AsyncWebServerRequest", "_response == NULL");
    _client->close(true);
    _onDisconnect();
    return;
  }
  if(!_response->_sourceValid()){
    ESP_LOGD("AsyncWebServerRequest", "!_response->_sourceValid()");
    delete response;
    _response = NULL;
    send(500);
  }
  else {
    ESP_LOGD("AsyncWebServerRequest", "else");
