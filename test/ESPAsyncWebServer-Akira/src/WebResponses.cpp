...
void AsyncWebServerResponse::_respond(AsyncWebServerRequest *request){ 
  ESP_LOGD("AsyncWebServerResponse", "_respond");
  _state = RESPONSE_END; 
...
void AsyncBasicResponse::_respond(AsyncWebServerRequest *request){
  ...
  ESP_LOGD("AsyncBasicResponse", "準備回傳訊息!");
  ESP_LOGD("AsyncBasicResponse", "out: %s, outLen: %d, space: %d, _contentLength: %d",out.c_str(),outLen, space, _contentLength);
...
AsyncFileResponse::~AsyncFileResponse(){
  if(_content) {
    ESP_LOGD("AsyncFileResponse", "File Close");
    _content.close();
  }
}
...
AsyncFileResponse::AsyncFileResponse(FS &fs, const String& path, const String& contentType, bool download, AwsTemplateProcessor callback): AsyncAbstractResponse(callback){
  ...
  ESP_LOGD("AsyncFileResponse", "_content.available : %d", _content.available());
  _contentLength = _content.size();
  ESP_LOGD("AsyncFileResponse", "_contentLength : %d", _contentLength);
  if(contentType == "")
...
size_t AsyncFileResponse::_fillBuffer(uint8_t *data, size_t len){
  ESP_LOGD("AsyncFileResponse", "Read File : %d", len);
